...
const char *const *
nm_dhcp_client_get_rejected_servers (NMDhcpClient *self)
{
	g_return_val_if_fail (NM_IS_DHCP_CLIENT (self), NULL);

	return (const char *const *) NM_DHCP_CLIENT_GET_PRIVATE (self)->rejected_servers;
}
...
nm_dhcp_client_server_id_is_rejected (NMDhcpClient *self, gconstpointer addr)
{
	...
	if (!priv->rejected_servers || !priv->rejected_servers[0])
		return FALSE;

	for (i = 0; priv->rejected_servers[i]; i++) {
		in_addr_t r_addr;
		guint32 mask;
		int r_prefix;

		nm_assert (nm_utils_parse_inaddr_prefix_bin (AF_INET,
		                                             priv->rejected_servers[i],
		                                             NULL,
		                                             &r_addr,
		                                             &r_prefix));
		mask = _nm_utils_ip4_prefix_to_netmask (r_prefix);
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_TIMEOUT:
		...
		break;
	case PROP_REJECTED_SERVERS:
		/* construct-only */
		priv->rejected_servers = g_value_dup_boxed (value);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
		break;
...
dispose (GObject *object)
{
	...
	if (priv->rejected_servers)
		g_strfreev (priv->rejected_servers);
...
nm_dhcp_client_class_init (NMDhcpClientClass *client_class)
{
	...
	obj_properties[PROP_REJECTED_SERVERS] =
	    g_param_spec_boxed (NM_DHCP_CLIENT_REJECTED_SERVERS, "", "",
	                        G_TYPE_STRV,
	                        G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
	                        G_PARAM_STATIC_STRINGS);
